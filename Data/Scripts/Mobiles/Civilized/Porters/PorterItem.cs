...
namespace Server.Items
{
	public class PackBeastItem : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( animalfriend == 0 )
			{
				from.SendMessage("Você precisa estar próximo a um druida ou tratador de animais para convocar este animal de carga!");
			}
			else
			{
				...
				{
					from.SendMessage("Você já possui um animal de carga.");
				}
				else if ( nFollowers > 0 )
				{
					from.SendMessage("Você já possui muitos seguidores no seu grupo.");
				}
				else if ( Charges < 1 )
				{
					from.SendMessage("Seu animal de carga precisa ser cuidado por um Mestre da Guilda dos Druidas.");
				}
				else if ( PorterOwner != from.Serial )
				{
					from.SendMessage("Este não é o seu animal de carga!");
				}
				else
				{
					Map map = from.Map;
