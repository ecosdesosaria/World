...
namespace Server.Items
{
	public class HenchmanFamiliarItem : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendMessage("Você já tem um familiar.");
			}
			else if ( nFollowers > 0 )
			{
				from.SendMessage("Você já tem pessoas demais em seu grupo.");
			}
			...
			{
				from.SendMessage("Apenas magos, elementalistas ou necromantes aprendizes podem invocar esses familiares.");
			}
			else if ( Charges < 1 )
			{
				from.SendMessage("Esta bola de cristal parece estar sem cargas.");
			}
			else if ( FamiliarOwner != from.Serial )
			{
				from.SendMessage("Esta não é sua bola de cristal!");
			}
			else
			{
				Map map = from.Map;
