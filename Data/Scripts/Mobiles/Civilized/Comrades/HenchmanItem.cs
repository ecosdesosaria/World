...
namespace Server.Items
{
	public class HenchmanItem : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendMessage("Eles estão mortos e você deve contratar um curandeiro para ressuscitá-los.");
			}
			else if ( pets.Count > 1 )
			{
				from.SendMessage("Você já tem ajudantes suficientes em seu grupo.");
			}
			...
			else if ( HenchmanFunctions.IsInRestRegion( from ) == true )
			{
				...
				from.SendMessage("" + friend.Name + " provavelmente aventurará com você por mais " + nTime.ToString() + " minutos.");
				...
				if ( !(this is HenchmanMonsterItem) )
				{
					...
					{
						case 0: friend.Say("Estou pronto para a aventura."); break;
						case 1: friend.Say("Para onde estamos indo?"); break;
						case 2: friend.Say("" + from.Name + ", estou aqui para servir."); break;
						case 3: friend.Say("Que os monstros temam nossa banda destemida."); break;
						case 4: friend.Say("Que masmorra ousamos explorar?"); break;
						case 5: friend.Say("Já esperei aqui por tempo demais."); break;
						case 6: friend.Say("Olá, " + from.Name + "."); break;
						case 7: friend.Say("Fama e fortuna aguardam!"); break;
					}
				}
				}
				else
				{
					from.SendMessage("Você só pode chamar seu ajudante de uma casa, estalagem ou taverna.");
				}
				}
				...
				public override void AddNameProperties(ObjectPropertyList list)
				{
					...
					if ( HenchDead > 0 )
					{
						...
						list.Add( 1049644, "Ressuscitar Por " + sLive + " de Ouro"); // PARENTHESIS
					}
				}
		...
		public static void ResurrectHenchman( Mobile from )
		{
			if ( from is PlayerMobile && from.Alive )
			{
				...
				if ( giveMessage )
				{
					from.SendMessage("Seus ajudantes foram ressuscitados.");
