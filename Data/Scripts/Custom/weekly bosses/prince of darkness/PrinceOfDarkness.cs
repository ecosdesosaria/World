...
{
	[CorpseName( "Cadável do Príncipe das Trevas" )]
	public class PrinceOfDarkness : BaseCreature
	{
		...
		private void PerformRageAttack( Mobile target )
		{
			...
			switch ( attackChoice  )
			{
				case 1: // Freezing blast
					{
						PublicOverheadMessage( MessageType.Regular, 0x21, false, "Sinta a nevasca de Ozz!" );
						PlaySound( 0x64F );
				...
				case 2: // Rage 2+: Laceration (bleed+poison)
					{
						PublicOverheadMessage( MessageType.Regular, 0x21, false, "Sangre por mim!" );
						PlaySound( 0x133 );
						...
						foreach ( Mobile m in eable )
						{
							if ( m != this && m.Player && m.Alive && CanBeHarmful( m ) && Server.Items.BaseRace.IsBleeder( m ) )
							{
								...
								if ( !isImmune )
								{
									...
									{
										m.LocalOverheadMessage( MessageType.Regular, 0x982, false, "Você está sangrando muito!" );
									}
				...
				case 3: // Rage 3: Void blast (Mana drain + damage + freezing)
					{
						PublicOverheadMessage( MessageType.Regular, 0x21, false, "O sol, a lua e as estrelas todos carregam minha marca!" );
						PlaySound( 0x228 );
		...
		private void SpawnCreature( Mobile target )
		{
			...
			switch( m_Rage )
			{
				...
				case 1: 
					newSummons = Utility.RandomMinMax( 4, 8 ); 
					song = "Todos a bordo! HahaHAha!"; 
					break;
				case 2: 
					newSummons = Utility.RandomMinMax( 3, 6 ); 
					song = "Lata para a lua!"; 
					break;
				case 3: 
					newSummons = Utility.RandomMinMax( 2, 4 );
					song = "Generais reunidos em suas massas!"; 
					break;
				default:
					newSummons = 2;
					song = "";
		...
		public override bool OnBeforeDeath()
		{
			...
			else 
			{
				...
				PublicOverheadMessage( MessageType.Regular, 0x21, false, "Mamãe...estou voltando para casa..." );
			}
