...
namespace Server.Items
{
	public class MagicSkeltonsKey : Item
	{
		...
		private class UnlockTarget : Target
		{
			...
			{
				bool trash = false;
					if ( Utility.RandomMinMax(1,10) == 1 ){ trash = true; }
				...
				else if ( targeted is ILockable )
				{
					...
					else if ( o.Locked )
					{
						...
						else if ( o.Locked && m_Key.ItemID != 0x3A75 )
						{
							...
							{
								Item item = (Item)targeted;
								if ( trash ){ from.SendMessage( "A chave abre a fechadura, desgastando a chave para uso futuro." ); }
								else { from.SendMessage( "A chave abre a fechadura." ); }
							}
